...
export function PosicoesView() {
  ...
  if (activePortfolioId && isLoading && positions.length === 0) {
    return <ViewLoading message="Carregando posições..." />;
  }
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <PageHeader title="Posições Abertas" subtitle="Acompanhe suas posições em tempo real" />

      {/* Stats Grid - Compact */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatItem
          icon={<Wallet className="w-4 h-4 text-action-primary" />}
        ...
        />
        <StatItem
          icon={<span className="text-sm font-bold text-text-primary">$</span>}
      ...
      {/* Funding Status Banner - Compact */}
      <Card className={cn("p-3 border transition-all duration-200",
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center",
              ...
              <Clock className={cn("w-4 h-4", stats.totalFunding >= 0 ? "text-status-success" : "text-status-error")} />
            </div>
            <div>
              <span className="text-xs text-text-secondary">Funding Rate Total (8h)</span>
              <p className={cn("text-base font-bold font-mono", stats.totalFunding >= 0 ? "text-status-success" : "text-status-error")}>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-text-primary">Detalhes das Posições</h2>
          <Button
            variant="ghost"
            size="sm"
          ...
          >
            <Loader2 className={cn("w-3.5 h-3.5 mr-1.5", isLoading && "animate-spin")} />
            Atualizar
          </Button>
        </div>
        ...
        {positions.length === 0 ? (
          <Card className="p-6 border border-border-default bg-surface-card text-center">
            <p className="text-sm text-text-secondary">Nenhuma posição aberta no momento.</p>
          </Card>
        ) : (
